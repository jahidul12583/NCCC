...
const TeacherProfile: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setLoading(true);
            try {
                const [teacherRes, coursesRes, batchesRes, salaryRes] = await Promise.all([
                    getTeacherById(teacherId),
                    getCourses(),
                    getBatches(),
                    getTeacherSalaryHistory(teacherId),
                ]);
                setTeacher(teacherRes || null);
                setEditedTeacher(teacherRes || null);
                setCourses(coursesRes.filter(c => c.teacherId === teacherId));
    ...
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        setEditedTeacher({ 
            ...editedTeacher, 
            [name]: name === 'salaryPerClass' ? Number(value) : value 
        });
    };
    ...
    const handleUpdateTeacher = async (e: React.FormEvent) => {
        ...
        if (!editedTeacher) return;
        try {
            const updated = await updateTeacher(editedTeacher);
            setTeacher(updated);
    ...
    const handleExportPdf = () => {
        ...
        html2canvas(input, { scale: 2, useCORS: true }).then(canvas => {
            ...
            pdf.save(`${teacher.name}-Profile.pdf`);
        });
    };
    ...
    const handleSavePhoto = async () => {
        if (!teacher || !photoPreview) return;
        try {
            const updated = await updateTeacher({ ...teacher, photoUrl: photoPreview });
            setTeacher(updated);
    ...
    const getBatchDetails = (batchId: string) => batches.find(b => b.id === batchId);
    ...
    if (!teacher) {
        return <div className="text-center py-10 text-red-500">Teacher not found.</div>;
    }
        ...
        <>
            <div ref={profileRef} className="space-y-8 p-1">
                    ...
                    <div className="flex flex-col md:flex-row items-center md:items-start md:space-x-6">
                        <div className="flex flex-col items-center md:items-start">
                             <div className="relative w-32 h-32 group">
                                <img src={photoPreview || teacher.photoUrl} alt={teacher.name} className="w-32 h-32 rounded-full object-cover border-4 border-primary" crossOrigin="anonymous" />
                        ...
                        <div className="mt-4 md:mt-0 text-center md:text-left">
                            <h1 className="text-3xl font-bold text-text-primary">{teacher.name}</h1>
                            <p className="text-text-secondary">{teacher.id}</p>
                            <p className="text-text-secondary mt-1">{teacher.qualification}</p>
                            <div className={`mt-2 inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${teacher.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                {teacher.status}
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    {/* Left Column: Personal Info & Subjects */}
                    <div className="lg:col-span-1 space-y-6">
                        <div className="bg-card-bg rounded-lg shadow-md p-6">
                            <h3 className="text-lg font-semibold text-text-primary border-b pb-2 mb-4">Contact Information</h3>
                            <ul className="space-y-3 text-sm">
                                <li className="flex items-center"><Mail size={16} className="mr-3 text-primary"/>{teacher.email}</li>
                                <li className="flex items-center"><Phone size={16} className="mr-3 text-primary"/>{teacher.whatsappNumber}</li>
                                <li className="flex items-center"><Home size={16} className="mr-3 text-primary"/>{teacher.address}</li>
                                <li className="flex items-center"><Calendar size={16} className="mr-3 text-primary"/>Joined: {new Date(teacher.createdAt).toLocaleDateString()}</li>
                            </ul>
                        </div>
                        <div className="bg-card-bg rounded-lg shadow-md p-6">
                            <h3 className="text-lg font-semibold text-text-primary border-b pb-2 mb-4">Subjects & Batches</h3>
                            <ul className="space-y-3 text-sm">
                                {courses.map(course => {
                                    const batch = getBatchDetails(course.batchId);
                                    return (
                                        <li key={course.id} className="flex items-start">
                                            <Book size={16} className="mr-3 text-primary mt-0.5"/>
                                            <div>
                                                <p className="font-semibold">{course.subject}</p>
                                                {batch && <p className="text-xs text-text-secondary">{batch.classLevel} ({batch.name})</p>}
                                            </div>
                                        </li>
                                    );
                                })}
                                 {courses.length === 0 && <li className="text-xs text-gray-500">No subjects assigned.</li>}
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-card-bg p-4 rounded-lg shadow-md text-center">
                                <Briefcase size={24} className="mx-auto text-primary mb-2"/>
                            ...
                            <div className="bg-card-bg p-4 rounded-lg shadow-md text-center">
                                <DollarSign size={24} className="mx-auto text-green-500 mb-2"/>
                            ...
                            <div className="bg-card-bg p-4 rounded-lg shadow-md text-center">
                                <DollarSign size={24} className="mx-auto text-red-500 mb-2"/>
                                <p className="text-2xl font-bold text-text-primary">৳{totalDue.toLocaleString()}</p>
                                ...
                                <table className="min-w-full">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase">Final Salary</th>
                                    ...
                                    <tbody>
                                        {salaryHistory.map(s => (
                                            <tr key={s.id} className="border-b">
                                                ...
                                                <td className="px-4 py-2 text-sm">৳{s.finalSalary.toLocaleString()}</td>
            ...
            {editedTeacher && (
                <Modal title="Edit Teacher Profile" isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)}>
                    <form onSubmit={handleUpdateTeacher} className="space-y-4">
                        <input type="text" name="name" value={editedTeacher.name} onChange={handleFormChange} placeholder="Full Name" className="w-full p-2 border rounded" required />
                        <input type="tel" name="whatsappNumber" value={editedTeacher.whatsappNumber} onChange={handleFormChange} placeholder="WhatsApp Number" className="w-full p-2 border rounded" required />
                        <textarea name="address" value={editedTeacher.address} onChange={handleFormChange} placeholder="Address" className="w-full p-2 border rounded" required rows={2}></textarea>
                        <input type="text" name="qualification" value={editedTeacher.qualification} onChange={handleFormChange} placeholder="Qualification" className="w-full p-2 border rounded" required />
                        <input type="number" name="salaryPerClass" value={editedTeacher.salaryPerClass} onChange={handleFormChange} placeholder="Salary (Per Class)" className="w-full p-2 border rounded" required />
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Status</label>
                            <select name="status" value={editedTeacher.status} onChange={handleFormChange} className="w-full p-2 border rounded bg-white">
